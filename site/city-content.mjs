...
import {byName,nearestTo,rankNote,base} from './geo.mjs';
...
export function cityPage(name){
 ...
 // Real geography, so each service-area page carries information that is only
 // true of that town: its distance and direction from the base, the ring it
 // falls in, and the neighbouring towns we also cover.
 const place=byName[name];
 const geoBlock=place?`<section><h2>Where ${esc(name)} sits in our service area</h2><p>${esc(name)} is roughly ${Math.round(place.km)} km ${esc(place.direction)} of our ${esc(base.name)} base, in ${esc(place.band.label)} — ${esc(rankNote(name))}.</p><p>Towns we also serve near ${esc(name)}: ${nearestTo(name).map(near=>`<a href="${slug(near.name)}.html">${esc(near.name)}</a>, ${Math.round(near.apart)} km away`).join(' · ')}.</p></section>`:'';
 const geoFaq=place?`<details><summary>How far is ${esc(name)} from your base?${glyph('chevron')}</summary><p>${esc(name)} is roughly ${Math.round(place.km)} km ${esc(place.direction)} of our ${esc(base.name)} base. We confirm travel arrangements and timing for your exact address before booking.</p></details>`:'';
 ...
 return `<section class="page-intro container"><nav class="breadcrumb" aria-label="Breadcrumb"><a href="/">Home</a> / <a href="areas-we-serve.html">Service areas</a> / ${esc(name)}</nav><span class="eyebrow">${home?'OUR OPERATING BASE':data.confirmed?'CONFIRMED SERVICE AREA':'COVERAGE ENQUIRY'}</span><h1>${home?'CleanNest cleaning services':data.confirmed?'Cleaning availability':'Cleaning enquiries'}<br>in ${esc(name)}</h1><p>${esc(intro)}</p></section>
 <section class="container prose">
 ${home?`<section><h2>CleanNest’s listed Jalandhar office</h2><address>${esc(address)}</address><p>Call <a href="tel:${businessInfo.telephone}">${businessInfo.displayPhone}</a> during ${businessInfo.hours}. Share your Jalandhar locality, access details and preferred date so the team can check availability and plan the visit.</p><p><a href="${businessInfo.map}" target="_blank" rel="noopener">Directions to the listed office</a> · <a href="contact.html">All contact options</a></p></section><section><h2>Cleaning for Jalandhar homes and businesses</h2><p>Use this page for Jalandhar-specific availability and contact information. The <a href="services.html">service directory</a> explains each cleaning category, while the <a href="pricing.html">central rate card</a> lists published starting prices and units.</p><p>For a single reference covering what cleaning costs in the city, what a deep clean includes and excludes, and the questions to ask before booking any company, read the <a href="blog/cleaning-services-jalandhar.html">Jalandhar cleaning services guide</a>.</p></section>`:''}
