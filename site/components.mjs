...
export const simpleProcess=()=>`<section class="process-wrap"><div class="section container"><div class="process-head"><div><span class="eyebrow">THREE STEPS. NOTHING COMPLICATED.</span><h2>A clean home,<br>made simple.</h2></div></div><div class="simple-steps">${steps.map(([icon,title,caption,href,cta])=>`<a class="step-card concentric" href="${href}"><span class="step-icon">${glyph(icon)}</span><span class="step-body"><h3>${title}</h3><p>${caption}</p><span class="step-cta"><span class="step-cta-label">${cta}</span>${glyph()}</span></span></a>`).join('')}</div></div></section>`;
export const simpleCare=()=>`<section class="care-section container"><div class="care-image"><img src="assets/img/editorial/curtains-v2.webp" srcset="assets/img/editorial/curtains-v2-440.webp 440w, assets/img/editorial/curtains-v2-660.webp 660w, assets/img/editorial/curtains-v2.webp 880w" sizes="(max-width: 900px) calc(100vw - 48px), 46vw" alt="A CleanNest team member tending freshly cleaned curtains in a sunlit room" width="880" height="660" loading="lazy"></div><div class="care-copy"><span class="eyebrow">YOUR HOME. IN GOOD HANDS.</span><h2>Good people.<br>Thoughtful care.</h2><div class="care-benefits">${[['people','Trained team'],['shield','Background checked'],['leaf','Family-safe products'],['redo','Free re-clean promise']].map(([icon,text])=>`<div class="care-benefit"><span>${glyph(icon)}</span><h3>${text}</h3></div>`).join('')}</div><a class="text-link" href="about.html">Get to know CleanNest ${glyph()}</a></div></section>`;
