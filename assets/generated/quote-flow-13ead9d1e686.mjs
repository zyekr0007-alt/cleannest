...
form.addEventListener('submit',e=>{e.preventDefault();collect();if(!form.reportValidity())return;
 ...
 if(step===2){if(!state.name.trim()){error('Please enter your name.');return;}if(!/^\+?[\d ()-]+$/.test(state.phone)||state.phone.replace(/\D/g,'').length<10||state.phone.replace(/\D/g,'').length>15){error('Please enter a valid mobile number.');return;}}
