...
// One decorative shine on first view; never delays content or repeats on hover.
if(!reduced.matches&&!navigator.connection?.saveData&&'IntersectionObserver'in window){
 const shine=new IntersectionObserver(entries=>{for(const entry of entries)if(entry.isIntersecting){entry.target.classList.add('stars-shine');shine.unobserve(entry.target);}},{threshold:.8});
 document.querySelectorAll('.stars').forEach(el=>shine.observe(el));
 reduced.addEventListener('change',()=>{if(reduced.matches){shine.disconnect();document.querySelectorAll('.stars-shine').forEach(el=>el.classList.remove('stars-shine'));}});
}
