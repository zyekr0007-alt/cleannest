...
const clients=buildQuoteClient(clientCatalog);
...
// Quick-question form. Submits to the worker, which emails the enquiry; the
// panel's own phone/email rows remain the no-JavaScript and failure path.
const contactForm=()=>`<form id="contact-form" class="contact-form" data-endpoint="${inquiryEndpoint}" data-turnstile="${turnstileSiteKey}"><h2>Send a quick question</h2><p class="small">Leave your number and we’ll call you back.</p><label class="field">Your name<input id="contact-name" name="name" autocomplete="name" required maxlength="80" placeholder="Your name"></label><label class="field">Mobile number<input id="contact-phone" name="phone" type="tel" autocomplete="tel" inputmode="tel" required maxlength="20" placeholder="Your contact number"></label><label class="field">What do you need?<textarea id="contact-message" name="message" maxlength="1500" placeholder="Tell us what needs cleaning"></textarea></label><div class="turnstile-slot" id="turnstile-slot"></div><button type="submit" class="button primary">Send my message</button><div id="contact-status" role="status"></div><p class="privacy-note">We use these details only to answer your question. <a href="privacy.html">Privacy policy</a></p></form>`;
function contact(){return pageIntro('WE’RE HERE TO HELP','Let’s talk<br>about your space.','A quick question or a whole-home reset. Our local team is ready to help.')+`<section class="container contact-grid"><div class="contact-panel"><span class="eyebrow">START A CONVERSATION</span><a href="tel:${businessInfo.telephone}">${icon('phone')}<span><small>Call or WhatsApp</small>${businessInfo.displayPhone}</span>${icon()}</a><a href="mailto:${businessInfo.email}">${icon('mail')}<span><small>Email us</small>${businessInfo.email}</span>${icon()}</a><div class="contact-line">${icon('clock')}<span><small>Working hours</small>${businessInfo.hours}</span></div><p class="small">Same-day appointments are subject to availability.</p>${quote('','Build a cleaning estimate')}${contactForm()}</div><div class="office-panel"><span class="eyebrow">OUR JALANDHAR OFFICE</span><h2>A local team,<br>close to home.</h2><address>${address}</address><a class="text-link" href="${businessInfo.map}" target="_blank" rel="noopener">Get directions ${icon('external')}</a><hr><p>Serving ${cities.join(', ')}.</p><a class="text-link" href="areas-we-serve.html">See service areas ${icon()}</a></div></section>${cta()}`;}
...
function quotePage(){return `<section class="container quote-layout" id="quote-builder" data-endpoint="${inquiryEndpoint}" data-turnstile="${turnstileSiteKey}"><div class="quote-main concentric"><ol class="quote-steps" aria-label="Quote progress"><li data-step-label="0">Services</li><li data-step-label="1">Details</li><li data-step-label="2">Contact</li><li data-step-label="3">Estimate</li></ol><form id="quote-form"><div id="quote-stage"></div><div class="quote-navigation"><button class="button secondary" type="button" id="quote-back">${icon('back')} Back</button><button class="button primary" type="submit" id="quote-next">Next ${icon()}</button></div></form><aside class="booking-expectations"><h2>What happens next?</h2><p>${esc(booking.termsBrief)}</p><div class="expectation-actions"><a class="button secondary sm" href="refund.html">Refund policy</a><a class="button secondary sm" href="faqs.html">FAQs</a></div></aside><noscript><p>The quote builder needs JavaScript. <a href="${businessInfo.whatsapp}">Get a quote on WhatsApp</a> or call <a href="tel:${businessInfo.telephone}">${businessInfo.displayPhone}</a>.</p></noscript></div></section>`+pageIntro('A FEW TAPS TO A FRESH START','Your home.<br>Your estimate.','Pick your services. We’ll take it from there.');}
...
for(const [file,p] of pages){
 ...
 // alone — otherwise "../pricing.html" became "../../pricing.html".
...
fs.writeFileSync('sitemap.xml','<?xml version="1.0" encoding="UTF-8"?>\n<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">'+[...pages.entries()].filter(([f,p])=>!['404.html','quote.html',...aliases].includes(f)&&!p.noindex).map(([f])=>f).map(f=>`<url><loc>https://cleannest.in/${f==='index.html'?'':f==='blog/index.html'?'blog/':f}</loc></url>`).join('')+'</urlset>\n');
