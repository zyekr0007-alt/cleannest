...
let menuTimer;
function closeMenu(){if(!mobileNav)return;clearTimeout(menuTimer);mobileNav.removeAttribute('data-open');mobileNav.inert=true;document.querySelector('main')?.removeAttribute('inert');document.querySelector('.footer')?.removeAttribute('inert');menu.setAttribute('aria-expanded','false');menu.setAttribute('aria-label','Open navigation');menuTimer=setTimeout(()=>{mobileNav.hidden=true;},220);}
menu?.addEventListener('click',()=>{const open=menu.getAttribute('aria-expanded')!=='true';if(!open)return closeMenu();clearTimeout(menuTimer);mobileNav.hidden=false;mobileNav.inert=false;document.querySelector('main')?.setAttribute('inert','');document.querySelector('.footer')?.setAttribute('inert','');menu.setAttribute('aria-expanded','true');menu.setAttribute('aria-label','Close navigation');void mobileNav.offsetHeight;requestAnimationFrame(()=>{if(menu.getAttribute('aria-expanded')==='true')mobileNav.setAttribute('data-open','');});});
...
if(!matchMedia('(prefers-reduced-motion: reduce)').matches&&'IntersectionObserver'in window){const observer=new IntersectionObserver(entries=>entries.forEach(entry=>{if(entry.isIntersecting){entry.target.classList.add('is-revealing');observer.unobserve(entry.target);}}),{threshold:0,rootMargin:'0px 0px -8% 0px'});document.querySelectorAll('.section-heading,.care-copy,.process-grid,.service-grid,.secondary-grid,.other-service-list,.result-carousel,.review-grid,.coverage-band,.faq-list,.rate-table,.city-page-links,.journal-grid,.detail-layout,.service-guidance,.closing-final').forEach(el=>observer.observe(el));}
